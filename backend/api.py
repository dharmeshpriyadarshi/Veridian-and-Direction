...
# =====================================================
# NEW: PHASE 19 MODEL EVALUATION ENDPOINT
# =====================================================
@app.get("/api/v1/metrics/meta-eval")
def get_meta_evaluation():
    """
    Returns the persistent model_evaluation_report.json containing RMSE, MAE, R2, and T-test info
    generated by the ml_engine/evaluator.py script.
    """
    import os
    import json
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    root_dir = os.path.dirname(current_dir)
    report_path = os.path.join(root_dir, "data", "outputs", "model_evaluation_report.json")
    
    if not os.path.exists(report_path):
        raise HTTPException(status_code=404, detail="Model evaluation report not found. Run evaluator.py first.")
